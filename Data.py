...
engine = create_engine('sqlite:///FantasyLeagueOfIreland.db',connect_args={'check_same_thread': False}, echo=False)  # echo=False
...
def getUser(emailInput, passwordInput):
    user = session.query(User).filter(and_(User.email==emailInput,User.password==passwordInput)).first()
    session.close()
    return user
